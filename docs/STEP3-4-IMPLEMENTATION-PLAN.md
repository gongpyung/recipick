...
// 개별 함수
function createExtraction(youtubeUrl: string, forceReExtract?: boolean): Promise<{ extractionId: string; status: string }>
...
function updateRecipe(id: string, data: RecipeUpdatePayload): Promise<{ id: string; updated: boolean; updatedAt: string }>
```
